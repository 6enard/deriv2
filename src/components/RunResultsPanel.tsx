  ...
  return (
    <div className="bg-bg-secondary flex flex-col h-full min-h-0">
        ...
        <div className="flex items-center flex-1">
          {TAB_DEFS.map(({ id, label, icon: Icon }) => (
            <button
              ...
              className={`relative flex-1 sm:flex-none flex items-center justify-center gap-2 px-3 sm:px-6 py-4 text-sm sm:text-base transition-colors ${tab === id ? 'font-bold text-text-primary' : 'text-text-secondary hover:text-text-primary'}`}
            >
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        {tab === 'summary' && (
          <SummaryView runStats={runStats} winRate={winRate} currency={currency} latest={latest} onReset={onResetStats} />
        )}
        ...
        {tab === 'journal' && (
          <JournalView journal={journal} journalEndRef={journalEndRef} onReset={onResetStats} onClear={onClearJournal} onDownload={downloadJournalTxt} />
        )}
      </div>
...
function SummaryView({ runStats, winRate, currency, latest, onReset }: { runStats: RunStats; winRate: number; currency: string; latest?: OpenContract; onReset: () => void }) {
  ...
  return (
    <div className="p-3 sm:p-5 space-y-4">
      <div className="rounded-xl border border-border-light bg-bg-primary/40 p-4 sm:p-6">
        <div className="flex items-center gap-1 mb-5">
          {[0, 1, 2, 3, 4].map((step) => <span key={step} className={`h-2 flex-1 rounded-sm ${step < 4 ? 'bg-brand-blue/60' : 'bg-bg-hover'}`} />)}
        </div>
        <div className="inline-flex rounded-lg bg-brand-blue/70 px-2 py-1 text-xs font-bold text-white mb-5">{currency}</div>
        <div className="grid grid-cols-2 gap-x-6 gap-y-5">
          <Quote label="Potential profit/loss" value={`${profit >= 0 ? '+' : ''}${profit.toFixed(2)}`} valueClass={profit >= 0 ? 'text-brand-blue' : 'text-brand-red'} />
        ...
        <div className="mt-6 pt-6 border-t border-border-default text-center text-sm sm:text-base text-text-secondary">{latest && !latest.is_sold ? 'Resale not offered' : 'No active contract'}</div>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-5 gap-y-6 bg-bg-tertiary/60 px-3 sm:px-5 py-5 sm:py-6">
        ...
        <BottomStat label="Profit/loss" value={`${runStats.totalProfit >= 0 ? '+' : ''}${runStats.totalProfit.toFixed(2)} ${currency}`} valueClass={runStats.totalProfit >= 0 ? 'text-brand-blue' : 'text-brand-red'} />
      </div>
...
function TransactionsView({ trades, currency, onDetails, onReset, onDownload }: { trades: OpenContract[]; currency: string; onDetails: (contract: OpenContract) => void; onReset: () => void; onDownload: () => void }) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between px-4 sm:px-7 py-3 border-b border-border-default text-xs sm:text-sm font-bold text-text-primary">
        <span>Type</span><span>Entry/Exit spot</span><span>Buy price and P/L</span>
      </div>
      <div>
        {trades.map((contract) => {
          ...
          return (
            <button key={contract.contract_id} onClick={() => onDetails(contract)} className="w-full grid grid-cols-[minmax(72px,0.8fr)_minmax(130px,1.2fr)_minmax(115px,1fr)] gap-3 items-center px-4 sm:px-7 py-4 border-b border-border-default text-left hover:bg-bg-tertiary transition-colors">
              ...
              <span className="text-right text-xs sm:text-sm tabular"><span className="block text-text-primary">{contract.buy_price.toFixed(2)} {currency}</span><span className={isPositive ? 'text-brand-blue' : 'text-brand-red'}>{isPositive ? '+' : ''}{contract.profit.toFixed(2)} {currency}</span></span>
            </button>
          )
        })}
      </div>
      ...
      <div className="flex gap-2 px-4 sm:px-7 py-3">
        <button onClick={onReset} className="flex items-center gap-1.5 text-xs text-text-secondary hover:text-text-primary"><RotateCcw className="w-3 h-3" /> Reset</button>
        <button onClick={onDownload} className="ml-auto flex items-center gap-1.5 text-xs text-text-secondary hover:text-text-primary"><Download className="w-3 h-3" /> Download</button>
      </div>
    </div>
  )
}
...
function JournalView({ journal, journalEndRef, onReset, onClear, onDownload }: { journal: JournalEntry[]; journalEndRef: RefObject<HTMLDivElement | null>; onReset: () => void; onClear: () => void; onDownload: () => void }) {
  return (
    <div className="p-3 sm:p-5">
      {journal.length === 0 ? <EmptyState icon={ScrollText} title="Journal is empty" description="Bot activity and execution messages will appear here." /> : <>
        <div className="space-y-1">{journal.map((entry, index) => <JournalRow key={index} entry={entry} />)}</div>
        <div className="flex items-center gap-3 mt-4 pt-3 border-t border-border-default"><button onClick={onReset} className="flex items-center gap-1.5 text-xs text-text-secondary hover:text-text-primary"><RotateCcw className="w-3 h-3" /> Reset</button><button onClick={onClear} className="flex items-center gap-1.5 text-xs text-text-secondary hover:text-text-primary"><Trash2 className="w-3 h-3" /> Clear</button><button onClick={onDownload} className="ml-auto flex items-center gap-1.5 text-xs text-text-secondary hover:text-text-primary"><Download className="w-3 h-3" /> Download</button></div>
...
function JournalRow({ entry }: { entry: JournalEntry }) {
  ...
  const message = match ? `${amount > 0 ? 'Profit' : amount < 0 ? 'Loss' : 'Sold'}: ${amount > 0 ? '+' : ''}${amount.toFixed(2)}` : entry.message
  return <div className="rounded-xl px-3 py-2.5 hover:bg-bg-tertiary"><div className="flex items-center gap-2 mb-1"><span className={`w-1.5 h-1.5 rounded-full ${color}`} /><span className="text-[10px] text-text-muted">{entry.time.toLocaleString()}</span></div><div className={`pl-3.5 text-xs ${match ? amount >= 0 ? 'text-brand-blue font-semibold' : 'text-brand-red font-semibold' : 'text-text-secondary'}`}>{message}</div></div>
}
...
function IconButton({ label, onClick, children }: { label: string; onClick: () => void; children: React.ReactNode }) { return <button onClick={onClick} title={label} className="w-8 h-8 rounded-lg flex items-center justify-center text-text-muted hover:text-text-primary hover:bg-bg-tertiary"><span className="w-4 h-4">{children}</span></button> }
