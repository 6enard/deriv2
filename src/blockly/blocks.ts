...
function tradeTypeMenu(
  ...
  /*
   * During XML construction the category can
   * temporarily be empty. In that situation return
   * every known trade type instead of returning an
   * empty dropdown and causing Blockly to reject the
   * XML value.
   */
  const options =
...
function contractTypeMenu(
  ...
  /*
   * During XML import trade type can temporarily
   * be empty. Fall back to every known contract type
...
defineBlock(
  ...
  function (block) {
    const originalDomToMutation =
      block.domToMutation

    block.domToMutation =
      function (xml: Element) {
        originalDomToMutation?.call(
          this,
          xml,
        )
