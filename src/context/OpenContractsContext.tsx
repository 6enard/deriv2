...
interface OpenContractsContextValue {
  openContracts: Record<number, OpenContract>
  openContractList: OpenContract[]
  subscribeToContract: (contractId: number) => void
  sellContract: (contractId: number) => Promise<void>
  refreshPortfolio: () => Promise<void>
}

const OpenContractsContext = createContext<OpenContractsContextValue | null>(null)

export function OpenContractsProvider({ children }: { children: ReactNode }) {
  ...
  const refreshPortfolio = useCallback(async () => {
    ...
    try {
      ...
      if (res.portfolio?.contracts) {
        for (const position of res.portfolio.contracts) {
  ...
  return (
    <OpenContractsContext.Provider value={{
      openContracts,
      openContractList,
      subscribeToContract,
      sellContract,
      ...
      {children}
    </OpenContractsContext.Provider>
  )
}

export function useOpenContracts(): OpenContractsContextValue {
  const ctx = useContext(OpenContractsContext)
  if (!ctx) throw new Error('useOpenContracts must be used within OpenContractsProvider')
  return ctx
}
