...
async function fetchAccounts(
  ...
  if (!res.ok) {
    ...
    throw new Error(
      err.error || `Accounts request failed (${res.status})`,
    )
  }
...
async function createAccount(
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({}))
    throw new Error(
      err.error || `Account creation failed (${res.status})`,
    )
  }
...
async function fetchOtpUrl(
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({}))
    throw new Error(
      err.error || `OTP request failed (${res.status})`,
    )
  }
    ...
    async (
      ...
      setAccounts((prev) => {
        const next = prev.map((a) =>
          a.account_id === updated.account_id
            ? updated
            : a,
        )
  ...
  const switchAccount = useCallback(
    async (accountId: string) => {
      ...
      try {
        ...
        setAccounts((prev) =>
          prev.map((a) =>
            a.account_id === refreshed.account_id
              ? refreshed
              : a,
          ),
        )
